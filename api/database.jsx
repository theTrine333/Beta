...
export const deletePlaylist = async (db, Title, loaderFunc) => {
  try {
    await db.runAsync(`DELETE FROM Playlists where Name="${Title}"`);
    await db.runAsync(`DELETE FROM PlayListItems WHERE Parent="${Title}"`);
    await loaderFunc();
  } catch (error) {}
};
